  ...
  try {
    ...
    if (error) {
      ...
      if (validQuestions.length === 0) {
        console.error('Supabaseから不完全なデータを受信しました。有効な questionJa を持つ行がありません。');
        showErrorBanner('クイズデータが不完全です。オフラインモードで実行します。(SupabaseのRLSポリシーでカラムへのアクセスが許可されているか確認してください)');
        useHardcoded = true;
      } else {
        if (validQuestions.length < data.length) {
          console.warn(`Supabaseから取得した ${data.length} 件のうち、${data.length - validQuestions.length} 件は無効なため除外されました。`);
        }
        console.log('取得したクイズ件数:', validQuestions.length);
        allQuestions = validQuestions.map((row) => {
          ...
          if (typeof account_options === 'string') {
            account_options = account_options.split(',').map(s => s.trim()).filter(Boolean);
          }
          ...
          return { ...row, solution, account_options, account_optionsEn, ref_links };
        });
      }
    }
